#pragma once

// GlobalSign Root R3. It currently validates both dashscope.aliyuncs.com and
// restapi.amap.com through the certificate chains served by those endpoints.
// Public root certificate, valid until 2029-03-18.
inline constexpr char GLOBALSIGN_ROOT_R3[] = R"CERT(
-----BEGIN CERTIFICATE-----

JQZR7rkpeDMdmztcpHWD9f
-----END CERTIFICATE-----
)CERT";
